...
# what does 26 multiplications mean?
#  - bit_length + num_1_bits
# what is the range?
#  - 0b11111111111111 - 0b100000000000000000000000000
#  - 16383 - 67108864
# go through and generate a list of number between 16383 - 67108864 that require
# 26 multiplications.
...
class hack_key():
    ...
    def bitcount(self, n):
        return (self.bitcount_lut[(n >> 24) & 0x00ff] + 
                self.bitcount_lut[(n >> 16) & 0x00ff] + 
                self.bitcount_lut[(n >> 8) & 0x00ff] + 
                self.bitcount_lut[n & 0x00ff])
    
    def bitlen(self, n):
        return int(log(n, 2)) + 1
    
    def fast_count_mul(self, n):
        return self.bitlen(n) + self.bitcount(n) - 2

    def keygen(self):
        for i in range(16383, 67108864+1):
            if self.fast_count_mul(i) == 26:
                yield i
    
    def find_key(self):
        for a in self.keygen():
            if mod_exp(g, a, p) == g_a:
                return a
    

hk = hack_key()
keys = [x for x in hk.keygen()]
print len(keys)
...
alices_key = 6301760
#g_ab = mod_exp(g_b, alices_key, p)
#print bits_to_string(decode(ciphertext, convert_to_bits(g_ab)))
