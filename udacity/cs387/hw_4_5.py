...
def encrypt(message, n, public_key, nonce, g, h):
    ...
    return convert_to_bits(mod_exp(m_int, public_key, n))
...
def decrypt(cipher, n, private_key):
    oaep_msg = pad_bits(convert_to_bits(mod_exp(cipher, private_key, n)), g+h)
    X = oaep_msg[:g]
    Y = oaep_msg[g:]
    r = xor(Y, hash(X, h))
    m = xor(X, hash(r, g))
    return bits_to_string(m)
...
print decrypt(bits_to_int(ciphertext), n, d)