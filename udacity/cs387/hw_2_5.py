...
def find_collision(message):
    ...
    h1 = counter_mode_hash(new_message)
    # find a cell 128 apart that is not equal
    for i in range(len(new_message) - 128):
        if new_message[i] != new_message[i + 128]:
            swp = new_message[i]
            new_message[i] = new_message[i + 128]
            new_message[i + 128] = swp
            break
    return new_message
    # END OF YOUR CODE
    ####################
