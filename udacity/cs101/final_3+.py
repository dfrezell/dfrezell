...
def multi_lookup(index, query):
    ...
    for i, word in enumerate(query):
        hits = lookup(index, word)
        if hits:
            for (url, pos) in hits:
                tmpdb[(i, pos, url)] = True

    for (i, pos, url) in tmpdb:
        if i > 1:
            continue
        match = True
        for x in range(len(query)):
            if not (i+x, pos+x, url) in tmpdb:
                match = False
        if match:
            result.append(url)
...
def add_page_to_index(index, url, content):
    ...
    for pos, word in enumerate(words):
        add_to_index(index, word, pos, url)
...
print multi_lookup(index, ['most', 'powerfuler', 'weapon'])
#>>> []

print multi_lookup(index, ['Monty', 'likes', 'the', 'Python', 'programming', 'language'])
#>>> ['http://www.udacity.com/cs101x/final/b.html']

print multi_lookup(index, ['about', 'programming', 'language'])
#>>> []
