...
class cfgstate():
    ...
    def isgoal(self):
        return all([c not in self.nonterminals for c in self.sentence])
    
    def expand(self):
        for pos in range(len(self.sentence)):
            for rule in self.grammar:
                if self.sentence[pos] == rule[0]:
                    yield (rule, self.sentence[0:pos] + rule[1] + self.sentence[pos + 1:])
                
    def predecessors(self):
        return [cfgstate(r, s) for r, s in self.expand()]
...
def cfgempty(grammar, symbol, visited):
    cfgstate.grammar = grammar
    cfgstate.nonterminals = [rule[0] for rule in grammar]
    start = cfgstate(None, [symbol])
    return treesearch([start])
