...
def is_utterance(sentence, nonterminals):
    return all([x not in nonterminals for x in sentence])
...
def isambig(grammar, start, utterance):
    ...
    while len(frontier):
        sentence = frontier.pop(0)
        if is_utterance(sentence, nonterminals):
            utterances.append(sentence)
            continue
        for s in expand(sentence, grammar):
            frontier.append(s)
        
    return len([u for u in utterances if u == utterance]) > 1
