...
def online_slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    take_omega = [x for x in range(2, edim)]
    exp_omega = [x for x in range(2, edim)]
    exp_omega[0] = 0
    exp_omega[1] = 1

    take_xi = [x for x in range(2, edim)]
    exp_xi = [x for x in range(2, edim)]
    exp_xi[0] = 0
    exp_xi[1] = 1
    ...
    for k in range(len(data)):
        ...
        # expand our matrix:
        Omega = Omega.expand(edim, edim, exp_omega, exp_omega)
        Xi = Xi.expand(edim, 1, exp_xi, [0])
        ...
        A = Omega.take([0, 1], take_omega)
        B = Omega.take([0, 1])
        C = Xi.take([0, 1], [0])
        Omega_p = Omega.take(take_omega)
        Xi_p = Xi.take(take_xi, [0])
