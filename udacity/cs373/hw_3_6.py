...
class robot:
    ...
    def move(self, motion): # Do not change the name of this function
        alpha, distance = float(motion[0]), float(motion[1])

        if distance < 0.0:
            raise ValueError, 'Robot cant move backwards'         
        if alpha > max_steering_angle:
            raise ValueError, 'Steering angle to big'

        # add in our sensor noise for distance and steering
        alpha = alpha + random.gauss(0.0, self.steering_noise)
        ...
        tan_alpha = tan(alpha)
        beta = (distance * tan_alpha) / self.length
        ...
        if beta < 0.001:
            ...
            R = self.length / tan_alpha
            c_x = x - R * sin(theta)
            c_y = y + R * cos(theta)
            x = c_x + R * sin(beta + theta)
            y = c_y - R * cos(beta + theta)
            theta += beta
    ...
    # sense: 
    #    
    def sense(self, enable = 1): #do not change the name of this function
        Z = []
        noise = 0.0
        for pos in landmarks:
            # landmarks are in (y, x) form
            d_x, d_y = pos[1] - self.x, pos[0] - self.y
            if enable:
...
#seed = 10 
#random.seed(seed) 
#print 'Test runs with seed: ', seed 
...
##    motions and measurements should return a [x,y,orientation]
##    vector near [x=100.0 y=50.0 orient=0.0], that is, the
##    robot's true location.`
##

##motions = [[0.0, 10.0] for i in range(11)]
##measurements = []
##
##test_robot = robot()
##test_robot.set(0.0, 50.0, 0.0)
##test_robot.set_noise(0.0, 0.0, 0.0)
##
##for m in motions:
##    test_robot = test_robot.move(m)
##    measurements.append(test_robot.sense())
##
##
##final_robot = robot()
##final_robot.set(100.0, 50.0, 0.0)
##estimated_position = particle_filter(motions, measurements)
##print 'Ground truth:    ', final_robot
##print 'Particle filter: ', estimated_position
##print 'Code check:      ', check_output(final_robot, estimated_position)
